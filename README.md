...
```sh
make install                            # native install to /system
make install DESTDIR=/mnt/rootfs        # staged install under DESTDIR
```
...
# Cargo
cargo build --release --locked --target x86_64-unknown-linux-musl
mkdir -p /mnt/rootfs/system/bin
cp target/x86_64-unknown-linux-musl/release/cps /mnt/rootfs/system/bin/

# Make
make build  RUST_TARGET=x86_64-unknown-linux-musl
make install RUST_TARGET=x86_64-unknown-linux-musl DESTDIR=/mnt/rootfs

# Meson
meson setup builddir --cross-file cross.txt --prefix=/system
meson compile -C builddir
meson install -C builddir --DESTDIR=/mnt/rootfs

# CMake
cmake -B build -DCMAKE_TOOLCHAIN_FILE=toolchain.cmake -DCMAKE_INSTALL_PREFIX=/system
cmake --build build
cmake --install build --prefix /mnt/rootfs/system
```
...
cargo build --release --locked --target aarch64-unknown-linux-musl
mkdir -p /mnt/rootfs/system/bin
cp target/aarch64-unknown-linux-musl/release/cps /mnt/rootfs/system/bin/

# Make
make build  RUST_TARGET=aarch64-unknown-linux-musl
make install RUST_TARGET=aarch64-unknown-linux-musl DESTDIR=/mnt/rootfs
